...
class Application(connectionEstablished: Observable[WebSocket]) {
  ...
  connectionEstablished addObserver { socket => // #CB
    ...
    socket send write(initialPosition) // #REMOTE-SEND
    socket send write(figures) // #REMOTE-SEND
  }
  ...
  def updateInitialPosition(modification: Modification) = modification match {
    case Create(figure) =>
      ...
      sockets foreach { _ send write(initialPosition) } // #REMOTE-SEND
  ...
  def updateFigures(modification: Modification) = {
    ...
    if (figuresUpdated)
      sockets foreach { _ send write(figures) } // #REMOTE-SEND
  }
}
